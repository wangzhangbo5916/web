...
- 没有直接替代：`componentWillMount`在组件挂载前调用，用于设置状态或执行副作用。使用此方法在新版`React`中被认为是不推荐的，因为它可能导致不一致的渲染结果。
...
- 不是显式调用：虽然`useEffect`可以返回一个清理函数，达到卸载的效果，但没有一个单独的方法。
- 替代方案：在`useEffect`中返回一个清理函数，这相当于`componentWillUnmount`的功能。
